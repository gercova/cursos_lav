                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold text-green-900 mt-1">
                                {{ $exams->where('is_active', true)->count() }}</p>
                        </div>
            ...
            <div class="overflow-x-auto md:overflow-visible" id="exams-table-container" x-show="!loading">
                @if ($exams->isEmpty())
                    ...
                    <div class="text-center py-16 px-6">
                        ...
                        <p class="text-gray-500 mb-6 max-w-md mx-auto">Comienza creando tu primer examen para evaluar a los
                            estudiantes.</p>
                        <button @click="showCreateModal()"
                                        ...
                                        <div class="flex items-center gap-4">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <div class="flex items-center gap-2 mb-1">
                                                    ...
                                                    @if ($exam->is_final)
                                                        <span
                                                            class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gradient-to-r from-red-100 to-red-200 text-red-800">
                                    ...
                                    <td class="px-6 py-5">
                                        <div class="space-y-2">
                                            <div class="flex items-center justify-between">
                                                ...
                                                <span
                                                    class="text-sm font-semibold text-gray-900">{{ $exam->passing_score }}%</span>
                                            </div>
                                            <div class="flex items-center justify-between">
                                                <span class="text-xs text-gray-500">Tiempo límite:</span>
                                                <span class="text-sm font-semibold text-gray-900">
                                                    {{ $exam->duration > 0 ? $exam->duration . ' min' : 'Ilimitado' }}
                                                </span>
                                            </div>
                                            <div class="flex items-center justify-between">
                                    ...
                                    <td class="px-6 py-5">
                                        <div class="space-y-2">
                                            <div class="flex items-center justify-between">
                                                ...
                                                <span
                                                    class="text-sm font-bold text-blue-600">{{ $exam->questions_count }}</span>
                                            </div>
                                            <div class="flex items-center justify-between">
                                                <span class="text-xs text-gray-500">Intentos totales:</span>
                                                <span
                                                    class="text-sm font-bold text-green-600">{{ $exam->attempts_count ?? 0 }}</span>
                                            </div>
                                            <div class="flex items-center justify-between">
                                                <span class="text-xs text-gray-500">Aprobados:</span>
                                                <span
                                                    class="text-sm font-bold text-green-600">{{ $exam->passed_count ?? 0 }}</span>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-5">
                                        <div x-data="{ open: false }" class="relative flex items-center justify-end">
                                            ...
                                            <button @click="open = !open"
                                                class="p-2 text-gray-600 hover:text-white hover:bg-gradient-to-r hover:from-gray-500 hover:to-gray-600 rounded-lg transition-all duration-200"
                                                title="Más acciones">
                                            ...
                                            <div x-show="open" @click.away="open = false"
                                                ...
                                                <button @click="toggleExamStatus({{ $exam->id }}); open = false"
                                                    class="w-full text-left px-4 py-2 text-sm flex items-center gap-2 hover:bg-{{ $exam->is_active ? 'amber' : 'green' }}-50 text-{{ $exam->is_active ? 'amber' : 'green' }}-600">
                                                    @if ($exam->is_active)
                                                ...
                                                <!-- Editar -->
                                                <button
                                                    @click="$dispatch('open-edit-modal', { id: {{ $exam->id }} }); open = false"
                                                ...
                                                <a href="{{ route('admin.exams.questions', $exam) }}"
                                                    class="block px-4 py-2 text-sm text-purple-600 hover:bg-purple-50 flex items-center gap-2">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                        viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="2"
                                                ...
                                                <!-- Eliminar -->
                                                <button @click="deleteExam({{ $exam->id }}); open = false"
                            ...
                            <div class="flex items-center justify-between">
                                <div>
                                    <h3 class="text-xl font-bold text-gray-900"
                                        x-text="isEditing ? 'Editar Examen' : 'Nuevo Examen'"></h3>
                                    <p class="text-sm text-gray-600 mt-1"
                                        x-text="isEditing ? 'Modifica la configuración del examen' : 'Completa todos los campos para crear un nuevo examen'">
                                    </p>
                                </div>
                            ...
                            <form @submit.prevent="submitForm" method="POST">
                                    ...
                                    <div
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                            ...
                                            <div>
                                                ...
                                                <div class="relative">
                                                    <input type="number" x-model="formData.passing_score" min="0"
                                                        max="100" required
                                                    ...
                                                    <span
                                                        class="absolute right-3 top-1/2 transform -translate-y-1/2 text-blue-600">%</span>
                                                </div>
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <div class="relative">
                                                    <input type="number" x-model="formData.duration" min="0"
                                                        placeholder="0 = sin límite"
                                                        class="w-full px-4 py-3 border border-blue-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition duration-200">
                                                    <span
                                                        class="absolute right-3 top-1/2 transform -translate-y-1/2 text-blue-600">min</span>
                                                </div>
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <div class="relative">
                                                    <input type="number" x-model="formData.max_attempts" min="0"
                                        ...
                                        <div class="mt-6 space-y-4">
                                            <div
                                                ...
                                                <div class="flex items-center">
                                                    <input type="checkbox" x-model="formData.show_results"
                                                        id="show_results"
                                            ...
                                            <div
                                                class="flex items-center justify-between p-4 bg-white rounded-xl border border-blue-200">
                                                <div class="flex items-center">
                                                    <input type="checkbox" x-model="formData.randomize_questions"
                                        ...
                                        <div class="flex items-center justify-between">
                                            <div>
                                                ...
                                                <p class="text-sm text-green-700 mt-1">
                                                    <span
                                                        x-text="formData.is_active ? 'Activo - Los estudiantes pueden tomarlo' : 'Inactivo - Solo visible en administración'"></span>
                                                </p>
                                            </div>
                                            <div>
                                ...
                                <div class="flex items-center justify-end gap-4 pt-6 mt-6 border-t border-gray-200">
                                    ...
                                    <button type="submit" :disabled="isSubmitting"
                                        ...
                                        <span
                                            x-text="isSubmitting ? 'Guardando...' : (isEditing ? 'Actualizar Examen' : 'Crear Examen')"></span>
                                    </button>
                                </div>
                            </form>
                        </div>
...
@section('scripts')
    <script>
        function examManager() {
            return {
                ...
                async performSearch() {
                    ...
                    const displayUrl =
                        `{{ route('admin.exams.index') }}?${params.toString().replace('&ajax=1','').replace('ajax=1&','').replace('ajax=1','')}`;
        ...
        function examModal() {
            return {
                ...
                async submitForm() {
                    ...
                    try {
                        const url = this.isEditing ? `/admin/exams/${this.formData.id}` :
        ...
        async function deleteExam(examId) {
            if (!confirm(
                    '¿Estás seguro de eliminar este examen? También se eliminarán todas las preguntas y resultados asociados.'
                )) {
                return;
            }
