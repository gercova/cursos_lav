...
@section('content')
    @php
        ...
        $resumeCourse =
            $inProgressCourses->filter(fn($c) => !is_null($c['last_accessed']))->sortByDesc('last_accessed')->first() ??
    ...
    <div class="progress-page" x-data="progressRing()" x-init="initRing()">
                ...
                <div class="flex flex-col lg:flex-row gap-6 items-start lg:items-center">
                    ...
                    <div class="flex-1">
                        <div class="flex items-center gap-2 mb-2">
                            <span class="text-xs font-semibold text-blue-300 uppercase tracking-widest">Panel de
                                aprendizaje</span>
                        </div>
                        ...
                        <p class="text-blue-200 text-sm font-light max-w-lg">
                            Aquí tienes un resumen completo de tu aprendizaje. Sigue así — cada lección te acerca más a tu
                            meta.
                        </p>
                        ...
                        <div class="flex flex-wrap gap-4 mt-5">
                            <div class="flex items-center gap-2">
                                ...
                                <div>
                                    <p class="text-white font-extrabold display-font text-xl leading-none">
                                        {{ $stats['streak_days'] }}</p>
                            ...
                            <div class="flex items-center gap-2">
                                <span class="text-emerald-400 text-lg"><i class="bi bi-check-circle"></i></span>
                                <div>
                                    <p class="text-white font-extrabold display-font text-xl leading-none">
                                        {{ $stats['completed_courses'] }}</p>
                                    <p class="text-blue-300 text-[11px] font-medium">cursos finalizados</p>
                                </div>
                            </div>
                            <div class="w-px bg-white/10 self-stretch hidden sm:block"></div>
                            <div class="flex items-center gap-2">
                                <span class="text-purple-400 text-lg"><i class="bi bi-clock"></i></span>
                                <div>
                                    <p class="text-white font-extrabold display-font text-xl leading-none">
                                        {{ $stats['total_study_hours'] }}</p>
                    ...
                    <div class="flex flex-col items-center gap-2 lg:pr-4">
                        <div class="donut-wrap">
                            <svg viewBox="0 0 140 140" width="140" height="140" class="progress-ring"
                                style="transform:rotate(-90deg)">
                                <circle class="track" cx="70" cy="70" r="58" stroke-width="10" />
                                <circle class="fill" cx="70" cy="70" r="58" stroke-width="10"
                                    stroke="#3b82f6" stroke-dasharray="364.4" :stroke-dashoffset="ringOffset"
                                    style="transition: stroke-dashoffset 1.4s cubic-bezier(0.34,1.1,0.64,1)" />
                            </svg>
                            <div class="donut-center">
                                <span class="display-font text-3xl font-extrabold text-white"
                                    x-text="displayRate + '%'"></span>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            <div class="stat-card amber anim-fadeUp delay-5">
                <div class="flex items-center gap-3 mb-3">
                    <div class="stat-icon amber"><i class="bi bi-fire"></i></div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div class="xl:col-span-2 space-y-6">
                ...
                @if ($resumeCourse)
                    <div class="resume-card p-5 anim-fadeUp delay-5">
                        <div class="relative z-10 flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                            <img src="{{ $resumeCourse['image_url'] }}" alt="{{ $resumeCourse['title'] }}"
                                class="resume-card-thumb">

                            <div class="flex-1 min-w-0">
                                <p class="text-blue-200 text-[11px] font-bold uppercase tracking-widest mb-1">
                                    <i class="bi bi-play-circle mr-1"></i> Retomar aprendizaje
                                </p>
                                ...
                                <div class="flex items-center gap-3 mb-4">
                                    <div class="flex-1"
                                        style="height:5px;background:rgba(255,255,255,0.2);border-radius:999px;overflow:hidden">
                                        <div
                                            style="height:100%;width:{{ $resumeCourse['progress'] }}%;background:rgba(255,255,255,0.9);border-radius:999px;transition:width 1.2s ease">
                                        </div>
                                    </div>
                                    <span
                                        class="text-white text-xs font-bold flex-shrink-0">{{ $resumeCourse['progress'] }}%</span>
                                </div>

                                <div class="flex items-center gap-2 flex-wrap">
                                    @if ($resumeCourse['last_lesson_id'])
                                        <a href="{{ route('lesson.show', [$resumeCourse['slug'], $resumeCourse['last_lesson_id']]) }}"
                                            class="btn-white">
                                            <i class="bi bi-play text-[9px]"></i> Continuar curso
                                        </a>
                                    @else
                                        <a href="{{ route('student.course.learn', $resumeCourse['id']) }}"
                                            class="btn-white">
                                            <i class="bi bi-play text-[9px]"></i> Continuar curso
                                        </a>
                                    @endif
                                    <span class="text-white/60 text-xs">
                                        {{ $resumeCourse['completed_lessons'] }} / {{ $resumeCourse['total_lessons'] }}
                    ...
                    <div class="flex flex-col sm:flex-row items-center gap-8">
                        <div style="width:180px;height:180px;flex-shrink:0;position:relative">
                            ...
                            <div
                                ...
                                <span
                                    class="display-font text-2xl font-extrabold text-slate-800">{{ count($courses) }}</span>
                        ...
                        <div class="flex-1 w-full space-y-4">
                            ...
                            <div>
                                ...
                                <div class="pbar-track">
                                    <div class="pbar-fill pbar-blue"
                                        style="width:{{ $stats['total_courses'] > 0 ? round(($inProgressCourses->count() / $stats['total_courses']) * 100) : 0 }}%">
                                    </div>
                                </div>
                            </div>
                            {{-- Completados --}}
                            <div>
                                ...
                                <div class="pbar-track">
                                    <div class="pbar-fill pbar-green"
                                        style="width:{{ $stats['total_courses'] > 0 ? round(($finishedCourses->count() / $stats['total_courses']) * 100) : 0 }}%">
                                    </div>
                                </div>
                            </div>
                            {{-- Sin iniciar --}}
                            <div>
                                ...
                                <div class="pbar-track">
                                    <div class="pbar-fill pbar-slate"
                                        style="width:{{ $stats['total_courses'] > 0 ? round(($notStartedCourses->count() / $stats['total_courses']) * 100) : 0 }}%">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div
                                ...
                                <span class="text-xs text-slate-500 font-semibold uppercase tracking-wide">Tasa de
                                    finalización global</span>
                                <span
                                    class="display-font text-lg font-extrabold text-blue-600">{{ $stats['completion_rate'] }}%</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="glass-card overflow-hidden anim-fadeUp delay-6" x-data="{ tab: 'inprogress' }">
                        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                            ...
                            <div class="filter-tabs">
                                <button class="filter-tab" :class="{ 'active': tab === 'inprogress' }"
                                ...
                                @if ($notStartedCourses->count() > 0)
                                    <button class="filter-tab amber" :class="{ 'active amber': tab === 'notstarted' }"
                                        @click="tab = 'notstarted'">
                                        Sin iniciar
                        ...
                        {{-- Tab: En progreso --}}
                        <div x-show="tab === 'inprogress'" x-transition:enter="transition ease-out duration-150"
                                        ...
                                        <div class="flex gap-4">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <div class="flex items-start justify-between gap-2 mb-1">
                                                    <h3
                                                        class="display-font text-sm font-bold text-slate-800 line-clamp-2 leading-snug">
                                                        {{ $course['title'] }}
                                                    </h3>
                                                    <span
                                                        class="flex-shrink-0 display-font text-sm font-extrabold
                                                ...
                                                <div class="flex items-center gap-3 mb-2.5 flex-wrap">
                                                    <p class="text-xs text-slate-400 flex items-center gap-1">
                                                        <i class="bi bi-chalkboard-user text-[10px]"></i>
                                                        {{ $course['instructor'] }}
                                                    </p>
                                                ...
                                                <div class="flex justify-end">
                                                    @if ($course['last_lesson_id'])
                                                        <a href="{{ route('lesson.show', [$course['slug'], $course['last_lesson_id']]) }}"
                                                            class="btn-primary">
                                                            <i class="bi bi-play text-[9px]"></i> Continuar
                                                        </a>
                                                    @else
                        ...
                        {{-- Tab: Sin iniciar --}}
                        @if ($notStartedCourses->count() > 0)
                            <div x-show="tab === 'notstarted'" x-transition:enter="transition ease-out duration-150"
                                x-transition:enter-start="opacity-0 translate-y-1"
                                x-transition:enter-end="opacity-100 translate-y-0">
                                <div class="divide-y divide-slate-100">
                                    @foreach ($notStartedCourses as $course)
                                        <div class="p-5 hover:bg-slate-50/60 transition-colors">
                                            <div class="flex gap-4">
                                                <div class="relative">
                                                    <img src="{{ $course['image_url'] }}" alt="{{ $course['title'] }}"
                                                        class="course-thumb opacity-75">
                                                    <div class="absolute inset-0 rounded-[10px] flex items-center justify-center"
                                                        style="background:rgba(15,23,42,0.35)">
                                                        <i class="bi bi-lock text-white text-sm opacity-80"></i>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    <div class="flex items-start justify-between gap-2 mb-1">
                                                        <h3
                                                            class="display-font text-sm font-bold text-slate-700 line-clamp-2 leading-snug">
                                                            {{ $course['title'] }}
                                                        </h3>
                                                    ...
                                                    <div class="flex justify-end">
                                                        <a href="{{ route('student.course.learn', $course['id']) }}"
                                                            class="btn-primary">
                                                            <i class="bi bi-play text-[9px]"></i> Iniciar curso
                                                        </a>
                                                    </div>
                                                </div>
                                            </div>
                ...
                @if ($finishedCourses->count() > 0)
                    <div class="glass-card overflow-hidden anim-fadeUp delay-7">
                        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                            <span class="section-title">
                                <i class="bi bi-check-double text-emerald-500"></i>
                                Cursos finalizados
                                <span
                                    class="text-[11px] font-bold bg-emerald-100 text-emerald-700 rounded-full px-2 py-0.5 ml-1">{{ $finishedCourses->count() }}</span>
                            </span>
                        </div>
                                    ...
                                    <div class="flex gap-4">
                                        <div class="relative flex-shrink-0">
                                            <img src="{{ $course['image_url'] }}" alt="{{ $course['title'] }}"
                                                class="course-thumb">
                                            <div
                                                class="absolute -top-1.5 -right-1.5 w-6 h-6 bg-emerald-500 rounded-full flex items-center justify-center shadow-sm">
                                                <i class="bi bi-check text-white" style="font-size:8px"></i>
                                            </div>
                                        </div>

                                        <div class="flex-1 min-w-0">
                                            <h3
                                                class="display-font text-sm font-bold text-slate-800 line-clamp-2 leading-snug mb-1">
                                                {{ $course['title'] }}
                                            </h3>
                                            ...
                                            <div class="flex items-center gap-3 mb-2.5 flex-wrap">
                                                ...
                                                @if ($course['last_accessed'])
                                                    <span class="last-accessed-chip">
                                                        <i class="bi bi-calendar-check text-[9px]"></i>
                                                        Completado
                                                        {{ \Carbon\Carbon::parse($course['last_accessed'])->diffForHumans() }}
                                                    </span>
                                                @endif
                                            ...
                                            <div class="flex items-center justify-between mb-3">
                                                <span class="text-[11px] text-emerald-600 font-semibold">
                                                    <i class="bi bi-circle-check mr-0.5 text-[10px]"></i>
                                                    100% · {{ $course['total_lessons'] }} lecciones
                                                </span>
                                                @if ($course['certificate_id'])
                                                    <span class="badge-pill badge-green"><i
                                                            class="bi bi-award text-[9px]"></i> Certificado</span>
                                                @elseif($course['has_exam'] && !$course['has_passed_exam'])
                                                    <span class="badge-pill badge-amber"><i
                                                            class="bi bi-circle-exclamation text-[9px]"></i> Examen
                                                        pendiente</span>
                                                @elseif($course['has_exam'] && $course['has_passed_exam'])
                                                    <span class="badge-pill badge-blue"><i
                                                            class="bi bi-star text-[9px]"></i> Examen aprobado</span>
                                                @else
                                            ...
                                            <div class="flex flex-wrap gap-2 justify-end">
                                                <a href="{{ route('student.course.learn', $course['id']) }}"
                                                    class="btn-secondary">
                                                    <i class="bi bi-redo text-[9px]"></i> Repasar
                                                </a>
                                                @if ($course['certificate_id'])
                                                    <a href="{{ url('/certificate/' . $course['certificate_id']) }}"
                                                        class="btn-emerald">
                                                        <i class="bi bi-certificate text-[9px]"></i> Ver certificado
                                                    </a>
                                                @elseif($course['has_exam'] && !$course['has_passed_exam'])
                                                    <a href="{{ url('/exams/' . $course['exam_id']) }}"
                                                        class="btn-amber">
                                                        <i class="bi bi-file-earmark-text text-[9px]"></i> Ir al examen
                                                    </a>
                                                @endif
                                            </div>
                                        </div>
            ...
            <div class="xl:col-span-1 space-y-5">
                {{-- ── Streak Card ── --}}
                <div class="anim-scale delay-5"
                    style="background:linear-gradient(135deg,#1e3a5f,#0f172a);border-radius:18px;padding:1.25rem 1.5rem;border:1px solid rgba(255,255,255,0.07);box-shadow:0 8px 28px -8px rgba(15,23,42,0.3)">
                    <div class="flex items-center justify-between mb-4">
                        <span class="text-blue-300 text-xs font-bold uppercase tracking-widest">Racha de estudio</span>
                        <span class="flame text-2xl"><i class="bi bi-fire text-amber-400"></i></span>
                    </div>
                    ...
                    @if ($recentActivity->count() > 0)
                            ...
                            @foreach ($recentActivity->take(10) as $loop_i => $activity)
                                @php
                                    ...
                                    $mainTitle =
                                        ...
                                        ($data['document_title'] ??
                                            ($data['exam_title'] ??
                                                ($data['course_title'] ?? ($activity->description ?? 'Contenido'))));
                                ...
                                <div
                                    class="activity-row flex items-start gap-3 px-4 py-3 border-b border-slate-50 hover:bg-slate-50/70 transition-colors {{ $loop_i === 0 ? 'bg-blue-50/30' : '' }}">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-center justify-between gap-1 mb-0.5">
                                            <span
                                                class="activity-type-label text-[9px] font-extrabold uppercase tracking-widest"
                                        ...
                                        @if ($courseTitle && $courseTitle !== $mainTitle)
                                            <p class="text-[11px] text-blue-500 font-medium mt-0.5 truncate">
                                                <i class="bi bi-graduation-cap text-[9px] mr-0.5"></i>{{ $courseTitle }}
                                            </p>
                                        @endif

                                        @if ($score !== null)
                                            <span
                                                class="mt-1 inline-flex items-center gap-1 text-[10px] font-bold rounded-full px-2 py-0.5
                                            {{ $passed ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-red-50 text-red-600 border border-red-200' }}">
                                                <i class="bi bi-star" style="font-size:8px"></i>
                        ...
                        <div class="scroll-fade-hint px-4 py-2 text-center border-t border-slate-100 bg-white">
                            <span class="text-[10px] text-slate-400 font-medium flex items-center justify-center gap-1">
                                <i class="bi bi-chevron-down text-[8px] text-slate-300"></i>
                                Scroll para ver más
                                <i class="bi bi-chevron-down text-[8px] text-slate-300"></i>
                            </span>
                        </div>
                ...
                @if ($certsCount > 0)
                    <div class="glass-card p-5 anim-scale delay-7">
                        <div class="section-header mb-4">
                            ...
                            <span
                                class="text-[11px] font-bold bg-amber-100 text-amber-700 rounded-full px-2 py-0.5">{{ $certsCount }}</span>
                        </div>
                        <div class="space-y-2">
                            @foreach (collect($courses)->filter(fn($c) => $c['certificate_id'])->take(3) as $c)
                                <a href="{{ url('/certificate/' . $c['certificate_id']) }}"
                                    ...
                                    <div
                                        class="w-8 h-8 bg-amber-400 rounded-lg flex items-center justify-center flex-shrink-0">
                                        <i class="bi bi-award text-white text-xs"></i>
                                    </div>
                                    <p
                                        class="text-xs font-semibold text-slate-700 line-clamp-2 flex-1 leading-snug group-hover:text-amber-700 transition-colors">
                                        {{ $c['title'] }}
                                    </p>
                                    <i class="bi bi-arrow-right text-amber-400 text-[10px] flex-shrink-0"></i>
                                </a>
                            @endforeach
                        </div>
                    </div>
