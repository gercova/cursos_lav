    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div class="flex items-center gap-2">
                ...
                @else
                    <span
                        class="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-100 text-gray-300 cursor-not-allowed"
                        title="No hay cursos programados en años futuros">
                        <i class="bi bi-chevron-right text-xs"></i>
                    </span>
                @endif
            </div>
        ...
        <div class="space-y-3">
            ...
            @foreach ($months as $num => $monthName)
                ...
                <div
                    class="bg-white rounded-xl border {{ $isCurrent ? 'border-blue-300 shadow-md' : ($isPast ? 'border-gray-200' : 'border-gray-100') }} overflow-hidden">
                    {{-- Cabecera del mes --}}
                    <div
                        class="px-5 py-3 flex items-center gap-3 {{ $isCurrent ? 'bg-gradient-to-r from-blue-600 to-blue-700 text-white' : ($isPast ? 'bg-gray-50' : 'bg-white') }}">
                        {{-- Indicador --}}
                        @if ($isCurrent)
                        ...
                        <h3
                            class="font-bold text-sm {{ $isCurrent ? 'text-white' : 'text-gray-800' }} uppercase tracking-wide">
                            {{ $monthName }} {{ $year }}
                        </h3>
                        ...
                        @elseif($isPast)
                            <span
                                class="text-xs bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full font-medium">Completado</span>
                        @else
                    ...
                    @if ($items->count() > 0)
                        <div class="divide-y divide-gray-50">
                            @foreach ($items as $item)
                                <div
                                    class="px-5 py-3 flex items-center gap-4 hover:bg-gray-50/50 transition-colors duration-150">
                                    {{-- Imagen del curso --}}
                                    ...
                                    <div class="flex-shrink-0 text-right">
                                        ...
                                        @elseif($isFuture)
                                            ...
                                            <span
                                                class="inline-flex items-center gap-1 text-xs text-gray-400 bg-gray-100 px-3 py-1.5 rounded-lg">
                                                <i class="bi bi-lock"></i>
                                                Próximamente
                                            </span>
                                        @else
                                            <span
                                                class="inline-flex items-center gap-1 text-xs text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-lg font-medium">
                    ...
                    @else
                        <div class="px-5 py-4 text-sm text-gray-400 italic">
                            <i class="bi bi-calendar-times mr-2"></i>Sin capacitaciones programadas para este mes.
                        </div>
                    @endif
                </div>
            @endforeach
