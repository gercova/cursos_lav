@foreach ($documents as $document)
    <tr class="document-row">
        ...
        <td class="px-6 py-4 whitespace-nowrap">
            <span
                class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800 uppercase">
                {{ $document->file_type }}
            </span>
        </td>
        ...
        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
            <div class="flex justify-end space-x-2">
                ...
                <button type="button" data-id="{{ $document->id }}"
                    class="duplicate-document-btn text-purple-600 hover:text-purple-900" title="Duplicar">
                    <i class="bi bi-copy"></i>
                </button>
                <a href="{{ route('admin.documents.edit', $document) }}" class="text-indigo-600 hover:text-indigo-900"
                    title="Editar">
                    <i class="bi bi-edit"></i>
                </a>
                <button type="button" data-id="{{ $document->id }}" data-title="{{ $document->title }}"
    ...
    <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between w-full">
        <div>
            <p class="text-sm text-gray-700">
                Mostrando <span class="font-medium">{{ $documents->firstItem() ?? 0 }}</span> a <span
                    class="font-medium">{{ $documents->lastItem() ?? 0 }}</span> de <span
                    class="font-medium">{{ $documents->total() }}</span> resultados
            </p>
        </div>
        <div>
