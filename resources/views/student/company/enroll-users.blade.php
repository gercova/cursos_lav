...
@section('content')
    <div class="container mx-auto px-4 py-4 sm:py-6" x-data="enrollManager()" x-init="init()">
                ...
                <div class="bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 rounded-xl p-4">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="bg-blue-600 p-2 sm:p-3 rounded-xl">
                            <i class="w-5 h-5 sm:w-6 sm:h-6 text-white bi-solid bi-people"></i>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xl sm:text-2xl font-bold text-green-900 mt-1">
                                {{ $totalCourses->where('type', 'course')->count() }}</p>
                        </div>
                        <div class="bg-green-600 p-2 sm:p-3 rounded-xl">
                            <i class="w-5 h-5 sm:w-6 sm:h-6 text-white bi-solid bi-book"></i>
                        </div>
                    </div>
                </div>
                <!-- Usuarios con código -->
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xl sm:text-2xl font-bold text-purple-900 mt-1">
                                {{ $totalCourses->where('id', '<>', $package->course_id)->where('type', 'package')->count() }}
                            </p>
                        </div>
                        <div class="bg-purple-600 p-2 sm:p-3 rounded-xl">
                            <i class="w-5 h-5 sm:w-6 sm:h-6 text-white bi-solid bi-tag"></i>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="flex items-start gap-2 sm:gap-3">
                        ...
                        <div class="flex-1">
                            <p class="text-xs sm:text-sm font-medium text-red-800">
                                ...
                                <a href="{{ route('student.package.select', $package->course_id) }}"
                                    class="underline font-semibold hover:text-red-900 block sm:inline mt-1 sm:mt-0">
        ...
        <div x-show="showSuperBulkConfirmModal" x-transition:enter="ease-out duration-300"
            ...
            <div class="flex items-center justify-center min-h-screen p-3 sm:p-4">
                <div
                    class="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-auto overflow-hidden transform transition-all">
                    ...
                    <div class="p-4 sm:p-6">
                        <div class="flex flex-col sm:flex-row items-center gap-3 sm:gap-4 mb-4">
                            <div
                                class="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-amber-100 flex items-center justify-center flex-shrink-0">
                                <i class="bi bi-rocket text-amber-600 text-lg sm:text-2xl"></i>
                            </div>
                            <div class="text-center sm:text-left">
                                <p class="text-base sm:text-lg font-semibold text-gray-900">¿Estás completamente seguro?</p>
                                <p class="text-xs sm:text-sm text-gray-600 mt-1">
                                    Vas a matricular <span
                                        class="font-bold text-amber-600">{{ $collaborators->count() }}</span> usuarios
                        ...
                        <div class="bg-amber-50 rounded-xl p-3 sm:p-4 mb-4">
                            <p class="text-xs sm:text-sm text-amber-800">
                                ...
                                <span
                                    class="text-xl sm:text-2xl font-bold text-amber-600">{{ $collaborators->count() * $courses->count() }}</span>
                                inscripciones
                            </p>
                        </div>
                        ...
                        <div class="flex flex-col sm:flex-row gap-3 mt-6">
                            <button @click="showSuperBulkConfirmModal = false"
                                class="w-full sm:flex-1 px-4 py-2 sm:py-3 border border-gray-300 text-gray-700 hover:bg-gray-50 rounded-xl font-medium transition duration-200 text-sm">
                                Cancelar
                            </button>
                            <button @click="executeSuperBulkEnroll(); showSuperBulkConfirmModal = false"
                                class="w-full sm:flex-1 px-4 py-2 sm:py-3 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-700 hover:to-orange-700 text-white font-semibold rounded-xl shadow-lg transition-all duration-200 text-sm">
                                Sí, ejecutar
                            </button>
                        </div>
                    </div>
        ...
        <!-- Modal de resultados -->
        <div x-show="showBulkResultsModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
            ...
            <div class="flex items-center justify-center min-h-screen p-3 sm:p-4">
                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] mx-auto overflow-hidden">
                    <div class="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white">
                        <div class="flex items-center justify-between">
                            <h3 class="text-base sm:text-xl font-bold text-gray-900"
                                x-text="bulkResults.title || 'Resultados'"></h3>
                            <button @click="showBulkResultsModal = false"
                                class="p-1.5 sm:p-2 hover:bg-gray-100 rounded-lg transition">
                    ...
                    <div class="p-4 sm:p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
                        <div class="mb-4">
                            <div
                                class="flex flex-col sm:flex-row items-center gap-3 sm:gap-4 p-3 sm:p-4 bg-green-50 rounded-lg">
                                <div
                                    class="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-green-100 flex items-center justify-center flex-shrink-0">
                                    <i class="bi bi-check-circle text-green-600 text-base sm:text-xl"></i>
                                </div>
                                <div class="text-center sm:text-left">
                                    <p class="text-sm sm:text-lg font-semibold text-green-800"
                                        x-text="bulkResults.message"></p>
                        ...
                        <div x-show="bulkResults.failed?.length > 0" class="mt-4">
                            ...
                            <div class="bg-red-50 rounded-lg p-3 sm:p-4 max-h-48 overflow-y-auto">
                                <template x-for="failed in bulkResults.failed" :key="failed.user + failed.course">
                                    <div
                                        class="text-xs sm:text-sm text-red-700 mb-2 pb-2 border-b border-red-100 last:border-0">
                                        <span class="font-medium" x-text="failed.user || 'Usuario'"></span>
                                        <span x-show="failed.course"> - <span class="text-xs"
                                                x-text="failed.course"></span></span>
        ...
        function enrollManager() {
            return {
                ...
                async enrollBulk() {
                    ...
                    if (!confirm(
                        `¿Estás seguro de matricular ${this.bulkForm.user_ids.length} usuarios en este curso?`)) {
                        return;
                    }
