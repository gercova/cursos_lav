...
<div class="container mx-auto px-4 py-4 sm:py-6" x-data="userManager()" x-init="init()">
    ...
    <div class="mb-6 sm:mb-8">
        <div class="flex flex-col sm:flex-row gap-3 sm:items-center justify-between mb-6">
            ...
            <a href="{{ route('company.create.new') }}" class="flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-semibold py-2.5 sm:py-3 px-4 sm:px-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 text-sm sm:text-base w-full sm:w-auto">
            ...
            <div class="bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 rounded-xl p-4 sm:p-5">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xl sm:text-2xl font-bold text-blue-900 mt-1">{{ $stats['seats_max'].' / '.$stats['total'] }}</p>
                    </div>
            ...
            <div class="bg-gradient-to-br from-green-50 to-green-100 border border-green-200 rounded-xl p-4 sm:p-5">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xl sm:text-2xl font-bold text-green-900 mt-1">{{ $total = (int) $stats['seats_max'] - (int) $stats['total'] }}</p>
                    </div>
    ...
    <div class="bg-white rounded-xl sm:rounded-2xl shadow-lg overflow-hidden border border-gray-200">
        ...
        <div x-show="!loading">
            @if($users->isEmpty())
                ...
                <div class="text-center py-12 sm:py-16 px-4 sm:px-6">
                    <div class="inline-flex items-center justify-center w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-gradient-to-br from-gray-100 to-gray-200 mb-4 sm:mb-6">
                    ...
                    <p class="text-sm sm:text-base text-gray-500 mb-4 sm:mb-6 max-w-md mx-auto">Comienza creando tu primer usuario para la plataforma.</p>
                    <a href="{{ route('company.create.new') }}" class="flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-semibold py-2.5 sm:py-3 px-4 sm:px-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 text-sm sm:text-base w-full sm:w-auto">
                        <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                        </svg>
                    ...
                    @foreach($users as $user)
                        <div class="p-4 hover:bg-gray-50 transition-colors">
                            ...
                            <div class="flex items-start gap-3 mb-3">
                                <div class="w-10 h-10 rounded-lg bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-white font-bold text-base flex-shrink-0">
                                    {{ strtoupper(substr($user->names, 0, 1)) }}
                                </div>
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-start justify-between gap-2">
                                        <div>
                                            <p class="font-semibold text-gray-900 text-sm truncate">{{ $user->names }}</p>
                                            <p class="text-xs text-gray-500">DNI: {{ $user->dni }}</p>
                                        </div>
                                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap bg-green-100 text-green-800">
                                            {{ $user->profession }}
                                        </span>
                                    </div>
                                    @if($user->code)
                                        <span class="inline-block mt-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-purple-100 text-purple-800">
                                            COD: {{ $user->code }}
                                        </span>
                                    @endif
                                </div>
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-2">
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold 
                                        ...
                                        <i class="bi {{ $user->is_active ? 'bi-check-circle-fill' : 'bi-x-circle-fill' }} mr-1"></i>
                                ...
                                <div x-data="{ open: false }" class="relative">
                                    <button @click="open = !open" class="p-2 text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition">
                                    ...
                                    <div x-show="open" @click.away="open = false"
                                        ...
                                        @if($user->id !== auth()->user()->id)
                                            <button @click="toggleUserStatus({{ $user->id }}); open = false" class="w-full flex items-center gap-3 px-4 py-2.5 text-sm {{ $user->is_active ? 'text-amber-600 hover:bg-amber-50' : 'text-emerald-600 hover:bg-emerald-50' }}">
                                                <i class="fas {{ $user->is_active ? 'fa-ban' : 'fa-check-circle' }} w-4"></i>
                                                {{ $user->is_active ? 'Desactivar' : 'Activar' }}
                                            </button>
                                        @endif

                                        {{-- <a href="{{ route('admin.users.show', $user) }}" class="block w-full px-4 py-2.5 text-sm text-blue-600 hover:bg-blue-50 flex items-center gap-3">
                                            <i class="fas fa-eye w-4"></i> Ver detalles
                                        </a> --}}

                                        {{-- <a href="{{ route('admin.users.edit', $user) }}" class="block w-full px-4 py-2.5 text-sm text-purple-600 hover:bg-purple-50 flex items-center gap-3">
                                            <i class="fas fa-edit w-4"></i> Editar
                                        </a> --}}

                                        <button @click="$dispatch('open-password-modal', { userId: {{ $user->id }} }); open = false" 
                                                class="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-amber-600 hover:bg-amber-50">
                                            <i class="fas fa-key w-4"></i> Cambiar contraseña
                                        </button>

                                        {{-- @if($user->code)
                                            <button onclick="copyPromoLink('{{ $user->code }}', this)" 
                                                    class="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-600 hover:bg-gray-50">
                                                <i class="fas fa-link w-4"></i> Copiar enlace
                                            </button>
                                        @else
                                            <button @click="$dispatch('open-code-user-modal', { userId: {{ $user->id }} }); open = false" 
                                                    class="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-emerald-600 hover:bg-emerald-50">
                                                <i class="fas fa-tag w-4"></i> Crear código
                                            </button>
                                        @endif --}}

                                        {{-- @if($user->id !== auth()->user()->id)
                                            <button @click="deleteUser({{ $user->id }}); open = false" 
                ...
                <div class="hidden sm:block overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50/80 backdrop-blur-sm">
                            <tr>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Usuario</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Contacto</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Cargo / Profesión</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Actividad</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider text-right">Acciones</th>
                            </tr>
                        </thead>
                                ...
                                <tr class="hover:bg-gradient-to-r hover:from-gray-50/50 hover:to-white transition-all duration-200 group">
                                    ...
                                    <td class="px-6 py-5">
                                        <div class="flex items-center gap-4">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                ...
                                                <div class="text-sm text-gray-500 truncate">DNI: {{ $user->dni }}</div>
                                                @if($user->code)
                                                    <div class="mt-1">
                                                        <span class="px-3 py-1 rounded-full text-xs font-semibold bg-gradient-to-r from-purple-100 to-purple-200 text-purple-800">
                                                            COD: {{ $user->code }}
                                                        </span>
                                                    </div>
                                                @endif
                                    ...
                                    <td class="px-6 py-5">
                                        <div class="flex flex-col gap-2">
                                            <span class="px-3 py-1 rounded-full text-xs font-semibold text-center bg-gradient-to-r from-green-100 to-green-200 text-green-800">
                                                @if($user->profession)
                                    ...
                                    <td class="px-6 py-5">
                                        <div x-data="{ open: false }" class="relative flex items-center justify-end">
                                            <button @click="open = !open" class="p-2 text-gray-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all duration-200 outline-none focus:ring-2 focus:ring-indigo-300">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z"/>
                                                </svg>
                                            </button>
                                            ...
                                            <div x-show="open" @click.away="open = false"
                                                ...
                                                <button @click="$dispatch('open-password-modal', { userId: {{ $user->id }} }); open = false" 
                                                    ...
                                                    <i class="fa-solid fa-key w-4"></i> Cambiar contraseña
                                                </button>

                                                {{-- @if($user->code)
                                                    <button onclick="copyPromoLink('{{ $user->code }}', this)"
                                                            class="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors duration-150">
                                                        <i class="fas fa-link w-4"></i> Copiar enlace
                                                    </button>
                                                @else
                                                    <button @click="$dispatch('open-code-user-modal', { userId: {{ $user->id }} }); open = false" 
                                                            class="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-emerald-600 hover:bg-emerald-50 transition-colors duration-150">
                                                ...
                                                {{-- @if($user->id !== auth()->user()->id)
                                                    <button @click="deleteUser({{ $user->id }}); open = false" 
                                                            class="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-red-600 hover:bg-red-50 transition-colors duration-150">
        ...
        <div x-show="loading" class="py-12 sm:py-16 text-center">
            <div class="inline-block animate-spin rounded-full h-8 w-8 sm:h-12 sm:w-12 border-t-2 border-b-2 border-blue-600 mb-3 sm:mb-4"></div>
                ...
                <div x-show="showModal"
                    ...
                    <div class="p-4 sm:p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
                        <form @submit.prevent="submitPassword">
                            ...
                            <div class="flex flex-col sm:flex-row items-center justify-end gap-3 pt-4 sm:pt-6 mt-4 sm:mt-6 border-t border-gray-200">
                                <button type="button" @click="closeModal" class="w-full sm:w-auto px-4 sm:px-6 py-2 sm:py-3 border border-gray-300 text-gray-700 hover:bg-gray-50 rounded-lg sm:rounded-xl font-medium transition duration-200 text-sm">
                                    Cancelar
                                </button>
                                <button type="submit" class="w-full sm:w-auto flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-semibold py-2 sm:py-3 px-6 sm:px-8 rounded-lg sm:rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 text-sm">
                                    <i class="bi bi-key"></i>
                                    Guardar cambios
                                </button>
                            </div>
                            ...
                            <div class="space-y-4 sm:space-y-6">
                                <div class="bg-gray-50 rounded-xl p-4 sm:p-6 border border-gray-200">
                                    <div class="mb-4">
                                        ...
                                        <input type="number" step="0.01" x-model="formData.discount_percentage" placeholder="Ej: 20" class="w-full px-3 sm:px-4 py-2 sm:py-3 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition duration-200">
                                        <p class="text-xs text-gray-500 mt-1">Si no se especifica, se usará 20% por defecto</p>
                                    </div>

                                    <div class="flex items-start gap-3">
                                        <div class="flex items-center h-5">
                                            <input type="checkbox" x-model="formData.promotion_price_is_active" id="promotion_price_is_active" class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                                        </div>
                                        <div class="flex-1">
                                            <label for="promotion_price_is_active" class="text-xs sm:text-sm font-medium text-gray-700">
                                                Código activo
                                            </label>
                                            <p class="text-xs text-gray-500">El código estará disponible para su uso</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    function copyPromoLink(code, button) {
        ...
        try {
            ...
            if (successful) {
                ...
                button.innerHTML = '<i class="fas fa-check text-green-600"></i> Copiado!';
