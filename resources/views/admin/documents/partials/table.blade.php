@if ($documents->count() > 0)
    @foreach ($documents as $document)
        <tr class="document-row">
            <td class="px-6 py-4">
                <div class="flex items-center">
                    <div class="flex-shrink-0 file-icon {{ $document->file_type }} mr-3">
                        <i
                            class="bi bi-file-earmark-{{ in_array($document->file_type, ['pdf'])
                                ? 'pdf'
                                : (in_array($document->file_type, ['doc', 'docx'])
                                    ? 'word'
                                    : (in_array($document->file_type, ['xls', 'xlsx'])
                                        ? 'excel'
                                        : (in_array($document->file_type, ['ppt', 'pptx'])
                                            ? 'powerpoint'
                                            : (in_array($document->file_type, ['jpg', 'jpeg', 'png', 'gif'])
                                                ? 'image'
                                                : 'default')))) }}"></i>
                    </div>
                    <div>
            ...
            <td class="px-6 py-4">
                <button
                    class="toggle-status-btn px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $document->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}"
                    data-id="{{ $document->id }}">
            ...
            <td class="px-6 py-4 text-right text-sm font-medium">
                <div class="flex justify-end space-x-2">
                    ...
                    <button class="delete-document-btn text-red-600 hover:text-red-900" data-id="{{ $document->id }}"
                        data-title="{{ $document->title }}" title="Eliminar">
...
@else
    <tr>
        <td colspan="7" class="px-6 py-12 text-center text-gray-500">
            <i class="bi bi-folder text-4xl mb-3"></i>
...
<div class="flex justify-between items-center">
    <div class="text-sm text-gray-700">
        Mostrando {{ $documents->firstItem() ?? 0 }} a {{ $documents->lastItem() ?? 0 }} de {{ $documents->total() }}
        resultados
    </div>
