...
@section('content')
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
            ...
            <div class="flex space-x-3 mt-4 md:mt-0">
                <button @click="openCreatePermissionModal()"
                    ...
                    <i class="bi bi-plus mr-2"></i> Nuevo Permiso
                </button>
                <a href="{{ route('admin.roles.index') }}"
                    class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg flex items-center">
                    <i class="bi bi-shield mr-2"></i> Gestionar Roles
                </a>
            </div>
        </div>
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div class="flex-1">
                    <div class="relative">
                        ...
                        <input type="text" x-model="searchTerm" @input="filterPermissions()"
                ...
                <div class="flex items-center space-x-4">
                    <div>
                        <select x-model="selectedModule" @change="filterPermissions()"
                        ...
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                <button @click="sortBy('name')" class="flex items-center hover:text-gray-700">
                                    Permiso
                                    <i class="bi bi-sort-down ml-1"
                                        :class="sortField === 'name' ? 'text-blue-500' : 'text-gray-400'"></i>
                                </button>
                            </th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                <button @click="sortBy('module')" class="flex items-center hover:text-gray-700">
                                    Módulo
                                    <i class="bi bi-sort-down ml-1"
                                        :class="sortField === 'module' ? 'text-blue-500' : 'text-gray-400'"></i>
                                </button>
                            </th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                <button @click="sortBy('action')" class="flex items-center hover:text-gray-700">
                                    Acción
                                    <i class="bi bi-sort-down ml-1"
                                        :class="sortField === 'action' ? 'text-blue-500' : 'text-gray-400'"></i>
                                </button>
                            </th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Descripción</th>
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900 max-w-xs truncate"
                                        x-text="permission.description || 'Sin descripción'"></div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                ...
                <div class="flex flex-col md:flex-row justify-between items-center">
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <template x-for="page in visiblePages" :key="page">
                            <button @click="goToPage(page)" class="px-3 py-1 rounded border"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                @foreach ($modules as $key => $module)
                    @php
                        ...
                        $percentage =
                            $permissions->count() > 0 ? ($modulePermissions->count() / $permissions->count()) * 100 : 0;
                    @endphp
    ...
    <div x-show="showPermissionModal" x-transition:enter="transition ease-out duration-300"
        ...
        <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <!-- Overlay -->
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" @click="showPermissionModal = false">
            </div>
                                ...
                                <div class="mt-4">
                                    <div class="grid grid-cols-2 gap-4 mb-4">
                                        <div>
                                            ...
                                            <select x-model="permissionForm.action"
                                            ...
                                            <p class="text-red-500 text-xs mt-1" x-show="permissionErrors.action"
                                                x-text="permissionErrors.action"></p>
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 mb-2">Módulo</label>
                                            <select x-model="permissionForm.module"
                                                class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                                            ...
                                            <p class="text-red-500 text-xs mt-1" x-show="permissionErrors.module"
                                                x-text="permissionErrors.module"></p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <div class="flex items-center space-x-2">
                                            <span
                                                class="px-3 py-2 bg-gray-100 rounded-l-md border border-r-0 border-gray-300 text-gray-600"
                                                x-text="(permissionForm.action ? permissionForm.action + '_' : '') + (permissionForm.module || '')"></span>
                                            <input type="text" x-model="permissionForm.customName"
                                        ...
                                        <p class="text-xs text-gray-500 mt-1">
                                            ...
                                            <span class="font-medium"
                                                x-text="(permissionForm.action ? permissionForm.action + '_' : '') + (permissionForm.module || '') + (permissionForm.customName ? '_' + permissionForm.customName : '')"></span>
                                        </p>
                                        <p class="text-red-500 text-xs mt-1" x-show="permissionErrors.name"
                                            x-text="permissionErrors.name"></p>
                                    </div>
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <textarea x-model="permissionForm.description" rows="3"
                                            class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                                            placeholder="Describe la funcionalidad de este permiso..."></textarea>
                                        <p class="text-red-500 text-xs mt-1" x-show="permissionErrors.description"
                                            x-text="permissionErrors.description"></p>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <div x-show="showConfirmModal" x-transition class="fixed inset-0 z-50 overflow-y-auto" x-cloak
        style="display: none;">
        <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" @click="showConfirmModal = false">
            </div>
                ...
                filterPermissions() {
                    ...
                    this.filteredPermissions = this.allPermissions.filter(permission => {
                        ...
                        const matchesSearch = !this.searchTerm ||
                            ...
                            (permission.description && permission.description.toLowerCase().includes(this.searchTerm
                                .toLowerCase()));
