                            ...
                            <tr class="hover:bg-gradient-to-r hover:from-gray-50/50 hover:to-white transition-all duration-200 group"
                                ...
                                <td class="px-6 py-5">
                                    <div class="inline-flex items-center gap-2">
                                        <span :class="{'bg-gradient-to-r from-green-100 to-green-200 text-green-800': {{ $category->is_active }}, 'bg-gradient-to-r from-red-100 to-red-200 text-red-800': !{{ $category->is_active }}}" class="px-3 py-1 rounded-full text-xs font-semibold">
    ...
    async function toggleStatus(categoryId) {
        ...
        try {
            const response = await axios.post(
                "{{ route('admin.categories.toggle-status', ['category' => ':categoryId']) }}".replace(':categoryId', categoryId)
            );
