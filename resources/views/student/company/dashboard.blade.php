...
@section('content')
    ...
    <div class="grid grid-cols-1 gap-6 mb-6">
        <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
            <h3 class="font-bold text-gray-700 mb-4 uppercase text-xs tracking-wider">Certificados Obtenidos (Últimos 6
                Meses)</h3>
            <div class="h-64">
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-6">
        <div class="p-5 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
            ...
            <span class="text-xs text-gray-500 bg-gray-100 px-3 py-1 rounded-full">
                <i class="bi bi-users mr-1"></i> {{ $collaboratorProgress->count() }} colaboradores
            </span>
        </div>
                    ...
                    @foreach ($collaboratorProgress as $index => $collab)
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            <td class="p-4 text-sm font-medium text-gray-500">
                                @if ($index == 0)
                                    <span class="text-yellow-500"><i class="bi bi-crown"></i></span>
                                @elseif($index == 1)
                                    <span class="text-gray-400"><i class="bi bi-medal"></i></span>
                                @elseif($index == 2)
                                    <span class="text-amber-600"><i class="bi bi-medal"></i></span>
                                @else
                                    {{ $index + 1 }}
                                @endif
                            </td>
                            <td class="p-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    @else
                                        <div
                                            class="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold text-xs shadow-sm">
                            ...
                            <td class="p-4 text-center">
                                <span
                                    class="text-sm font-bold {{ $collab['completed'] == $collab['total_courses'] && $collab['total_courses'] > 0 ? 'text-emerald-600' : 'text-gray-700' }}">
                                    {{ $collab['completed'] }}/{{ $collab['total_courses'] }}
                                </span>
                                @if ($collab['total_courses'] > 0 && $collab['completed'] == $collab['total_courses'])
                                    <i class="bi bi-check-circle text-emerald-500 ml-1 text-xs"></i>
                                @endif
                            </td>
                            <td class="p-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <span
                                        class="text-xs font-bold {{ $collab['avg_progress'] >= 80 ? 'text-emerald-600' : ($collab['avg_progress'] >= 50 ? 'text-amber-600' : 'text-gray-500') }}">
                                        {{ $collab['avg_progress'] }}%
                                    </span>
                                </div>
                            </td>
        ...
        @if ($collaboratorProgress->count() > 0)
            <div class="p-4 border-t border-gray-100 bg-gray-50 flex justify-between items-center text-xs text-gray-500">
                <div class="flex items-center gap-4">
                    <span><i class="bi bi-chart-line mr-1"></i> Promedio general: <strong
                            class="text-gray-700">{{ round($collaboratorProgress->avg('avg_progress'), 1) }}%</strong></span>
                    <span><i class="bi bi-check-circle mr-1 text-emerald-500"></i> Completaron todo: <strong
                            class="text-gray-700">{{ $collaboratorProgress->where('completed', $collaboratorProgress->where('total_courses', '>', 0)->sum('total_courses'))->count() }}</strong></span>
                </div>
                <div>
                    <i class="bi bi-arrow-up mr-1 text-emerald-500"></i> Ordenado por mejor progreso
                </div>
            </div>
        @endif
    </div>
