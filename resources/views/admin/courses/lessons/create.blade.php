...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-200">
        <form action="{{ route('admin.courses.sections.lessons.store', [$course, $section]) }}"
              method="POST"
              id="lessonForm"
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text"
                               ...
                               placeholder="Ej: Introducción al tema principal">
                        @error('title')
                    ...
                    <div>
                        ...
                        <input type="number"
                               id="order"
                               name="order"
                               value="{{ old('order', $section->lessons()->count() + 1) }}"
                               required min="1"
                               class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition duration-200">
                        @error('order')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <textarea id="description"
                              ...
                              rows="4"
                              class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition duration-200"
                              placeholder="Describe el contenido de esta lección, objetivos de aprendizaje, temas cubiertos, etc.">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
