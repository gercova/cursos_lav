...
@section('content')
    <div class="container mx-auto px-4 py-6" x-data="userManager()" x-init="init()">
            ...
            <div class="overflow-x-auto md:overflow-visible" id="users-table-container" x-show="!loading">
                @if ($users->isEmpty())
                    ...
                    <div class="text-center py-16 px-6">
                        ...
                        <p class="text-gray-500 mb-6 max-w-md mx-auto">Comienza creando tu primer usuario para la
                            plataforma.</p>
                        <a href="{{ route('admin.users.create') }}"
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @foreach ($users as $user)
                                <tr class="hover:bg-gradient-to-r hover:from-gray-50/50 hover:to-white transition-all duration-200 group">
                                        ...
                                        <div class="flex items-center gap-4">
                                            ...
                                            <div class="flex-shrink-0">
                                                @if ($user->profile_photo_url)
                                                    <img src="{{ $user->profile_photo_url }}"
                                                        class="w-12 h-12 rounded-xl bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center border-2 border-gray-300 group-hover:scale-110 transition-transform duration-200">
                                                @else
                                                    <div
                                            ...
                                            <div class="flex-1 min-w-0">
                                                ...
                                                @if ($user->code)
                                                    <div class="text-sm text-gray-500 truncate">
                                                        <span
                                                            class="px-3 py-1 rounded-full text-xs font-semibold text-center bg-gradient-to-r from-purple-100 to-purple-200 text-purple-800">
                                                            COD: {{ $user->code }}
                                                        </span>
                                                    </div>
                                                @endif
                                                <div class="flex items-center gap-3 mt-2 text-xs text-gray-400">
                                                    ...
                                                    <div class="flex items-center gap-1 {{ $user->expires_at && $user->expires_at->isPast() ? 'text-red-500 font-semibold' : '' }}"
                                                        title="Fecha de vencimiento">
                                                        <i class="bi bi-calendar-x"></i>
                                                        Vence:
                                                        {{ $user->expires_at ? $user->expires_at->format('d/m/Y') : 'Sin límite' }}
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    <!-- Acciones -->
                                    <td class="px-6 py-5 relative" x-data="{ open: false }" :class="{ 'z-30': open }">
                                        <div class="relative flex items-center justify-end">
                                            ...
                                            <div x-show="open" @click.away="open = false"
                                                ...
                                                <!-- Botón modal actualizar vencimiento -->
                                                <button
                                                    @click="$dispatch('open-expiration-modal', { userId: {{ $user->id }}, expiresAt: '{{ $user->expires_at ? $user->expires_at->format('Y-m-d') : '' }}' })"
                                                    class="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-blue-600 hover:bg-blue-50 transition-colors duration-150">
                                                    <i class="fa-solid fa-calendar-days"></i> Expiración
                                                </button>
                                                ...
                                                @if ($user->code == null)
                                                    @if ($user->role !== 'business')
                                                        <button
                                                            @click="$dispatch('open-code-user-modal', { userId: {{ $user->id }} })"
                                                ...
                                                @if ($user->role == 'business')
                                                    @if (!empty($user->companyPolicies->quantity))
                                                        <button
                                                            @click="$dispatch('open-company-policies', { userId: {{ $user->id }} })"
        ...
        <div x-data="passwordModal()" x-on:open-password-modal.window="handleOpen($event.detail)" x-cloak>
            <!-- Modal overlay -->
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <div class="relative">
                                                <input :type="showPassword ? 'text' : 'password'"
                                                    ...
                                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition duration-200 pr-10">
        ...
        <div x-data="expirationModal()" x-on:open-expiration-modal.window="handleOpen($event.detail)" x-cloak>
            <!-- Modal overlay -->
            <div x-show="showModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
                x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200"
                x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
                ...
                <div class="flex items-center justify-center min-h-screen p-4">
                    <div x-show="showModal" x-transition:enter="ease-out duration-300"
                        ...
                        <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white">
                            <div class="flex items-center justify-between">
                                <div>
                                    <h3 class="text-xl font-bold text-gray-900">Actualizar fecha de vencimiento</h3>
                                </div>
                                <button @click="closeModal"
                                    class="p-2 hover:bg-gray-100 rounded-lg transition duration-200">
                                    <svg class="w-5 h-5 text-gray-500" fill="none" stroke="currentColor"
                        ...
                        <!-- Contenido del modal -->
                        <div class="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
                            <form @submit.prevent="submitExpiration" id="expirationForm">
                                @csrf
                                <div class="space-y-6">
                                    <div
                                        class="bg-gradient-to-br from-gray-50 to-white rounded-xl p-6 border border-gray-200">
                                        <h4 class="text-lg font-semibold text-gray-900 mb-4">Información de Vencimiento
                                        </h4>
                                        <div class="mb-4">
                                            <label class="block text-sm font-medium text-gray-700 mb-2">
                                                Fecha de vencimiento *
                                            </label>
                                            <input type="date" x-model="formData.expires_at" required
                                                min="{{ date('Y-m-d') }}"
                                            ...
                                            <p class="text-xs text-gray-500 mt-1">La fecha de vencimiento es obligatoria y
                                                debe ser igual o posterior a hoy.</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="flex items-center justify-end gap-4 pt-6 mt-6 border-t border-gray-200">
                                    ...
                                    <button type="submit" :disabled="isSubmitting"
                                        ...
                                        <i x-show="!isSubmitting" class="fa-solid fa-calendar-check"></i>
                                        Guardar cambios
                                    </button>
                                </div>
                            </form>
        ...
        <div x-data="createCodeModal()" x-on:open-code-user-modal.window="handleOpen($event.detail)" x-cloak>
            <!-- Modal overlay -->
            <div x-show="showModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
                x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200"
                x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
                        ...
                        <div class="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
                            <form @submit.prevent="submitPassword" id="passwordForm">
                                ...
                                <div class="space-y-6">
                                    ...
                                    <div
                                        class="bg-gradient-to-br from-gray-50 to-white rounded-xl p-6 border border-gray-200">
                                        <h4 class="text-lg font-semibold text-gray-900 mb-4">Información de esta acción
                                        </h4>
    ...
    <script>
        function userManager() {
            return {
                ...
                async performSearch() {
                    ...
                    const displayUrl =
                        `{{ route('admin.users.index') }}?${params.toString().replace('&ajax=1','').replace('ajax=1&','').replace('ajax=1','')}`;
        ...
        function expirationModal() {
            return {
                ...
                handleOpen(detail) {
                    this.userId = detail.userId;
                    this.formData.expires_at = detail.expiresAt || '';
                    this.showModal = true;
                },
                ...
                closeModal() {
                    this.showModal = false;
                    this.userId = null;
                    this.formData.expires_at = '';
                },

                async submitExpiration() {
                    ...
                    if (this.formData.expires_at < todayStr) {
                    ...
                    try {
                        const response = await axios.put(
                            `/admin/users/${this.userId}/expiration`,
                            this.formData, {
                                headers: {
                                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute(
                                        'content')
                        ...
                        const message = error.response?.data?.message || 'Error al actualizar la fecha de vencimiento';
                        showNotification(message, 'error');
                    } finally {
                        this.isSubmitting = false;
                    }
        ...
        function showNotification(message, type = 'success') {
            ...
            notification.className = `fixed top-6 right-6 z-50 px-6 py-4 rounded-xl shadow-xl transform transition-all duration-300 ${type === 'success'
